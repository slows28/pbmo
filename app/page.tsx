...
function timeRangeParts(t: ActionTemplate) {
  const s = clampTime(String(t.start_time ?? t.default_time ?? "09:00"));
  const e = clampTime(String(t.end_time ?? "10:00"));
...
export default function Home() {
  const [theme, setTheme] = useState<ThemeKey>(() => {
    ...
    return (localStorage.getItem(LAST_THEME_KEY) as ThemeKey) || "dark";
  });

  const [tab, setTab] = useState<TabKey>(() => {
    if (typeof window === "undefined") return "actions";
    return (localStorage.getItem(LAST_TAB_KEY) as TabKey) || "actions";
  });
  ...
  useEffect(() => localStorage.setItem(LAST_THEME_KEY, theme), [theme]);
  useEffect(() => localStorage.setItem(LAST_DATE_KEY, dateKey), [dateKey]);
  ...
  function beginEdit(t: ActionTemplate) {
    ...
    setEditStart(clampTime(String(t.start_time ?? t.default_time ?? "09:00")));
    setEditEnd(clampTime(String(t.end_time ?? "10:00")));
  }
  ...
  async function saveEdit(id: string) {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok || !json?.ok) throw new Error(json?.error || "수정 저장 실패");
  ...
  async function addTemplate() {
    ...
    try {
      ...
      const start_time = clampTime(newStart);
      const end_time = clampTime(newEnd);
  ...
  async function deleteTemplate(id: string) {
    const ok = confirm("이 행동(템플릿)을 삭제할까요? 해당 행동의 모든 기록도 함께 삭제됩니다.");
    if (!ok) return;
  ...
  const sortedTemplates = useMemo(() => {
    ...
    copy.sort((a, b) => {
      ...
      const at = clampTime(String(a.start_time ?? a.default_time ?? "09:00"));
      const bt = clampTime(String(b.start_time ?? b.default_time ?? "09:00"));
  ...
  const palette = useMemo(() => {
    if (theme === "light") {
      return {
        ...
        chip2: "bg-zinc-100 ring-1 ring-zinc-200 text-zinc-700",
    ...
    return {
      ...
      chip2: "bg-white/5 ring-1 ring-white/10 text-white/70",
  ...
  const weekCards: Array<{ cat: Category }> = [{ cat: "운동" }, { cat: "공부" }, { cat: "기타" }];
            ...
            <section className={cn("mt-4 rounded-3xl p-3", palette.card)}>
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-[1fr_140px_140px_140px_88px] sm:items-center">
                ...
                <button
                  ...
                  disabled={loading}
                  className={cn(
                    "rounded-2xl px-3 py-2.5 text-[14px] font-semibold transition disabled:opacity-50",
                    theme === "light"
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        >
                          <div className={cn("h-full w-full", done ? "block" : "hidden")}>
                            <div className={cn("flex h-full w-full items-center justify-center text-[12px] font-black", theme === "light" ? "text-white" : "text-zinc-950")}>
                              ✓
                            </div>
                          </div>
                        </button>
                              ...
                              <div className="flex items-start justify-between gap-3">
                                ...
                                <div className="min-w-0">
                                  <div className="flex flex-wrap items-center gap-2">
                                    <div className={cn("text-[15px] font-semibold leading-6", done && (theme === "light" ? "line-through text-zinc-400" : "line-through text-white/50"))}>
                                      {t.name}
                                    </div>
                ...
                return (
                  <div key={cat} className={cn("rounded-3xl p-3", palette.subcard)}>
                    ...
                    <div className={cn("mt-2 h-2 overflow-hidden rounded-full", theme === "light" ? "bg-zinc-200" : "bg-white/10")}>
                      <div className={cn("h-full rounded-full", theme === "light" ? "bg-zinc-900" : "bg-sky-400")} style={{ width: `${pct}%` }} />
                    </div>
